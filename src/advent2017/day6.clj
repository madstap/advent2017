...
(def input (->> (-> (slurp "resources/day6.txt") (str/split #"\s"))
                (mapv comfy/str->int)))

(defn position-of-largest [coll]
  (let [x (apply max coll)]
    (reduce-kv (fn [_ i v]
                 (when (= x v)
                   (reduced i)))
               nil, coll)))
...
(defn redistribute [blocks]
  (let [largest (position-of-largest blocks)]
    (loop [cache (get blocks largest)
           blocks (assoc blocks largest 0)
           pos largest]
      (if (zero? cache)
        blocks
        (let [new-pos (if (contains? blocks (inc pos))
                        (inc pos)
                        0)]
          (recur (dec cache)
                 (update blocks new-pos inc)
                 new-pos))))))
