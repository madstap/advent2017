...
(def parse-line
  (insta/parser
   "<line> = name <ws> weight <ws> <arrow?> <ws> children+?
children = name <','>? <ws>
arrow = '->'
ws = #'\\s*'
<name> = #'[a-z]+'
...
(defn parse [s]
  (->> (str/split-lines s)
       (map parse-line)
       (map (fn [[n w & children]]
              (-> {:name n, :weight (comfy/str->int w)}
                  (comfy/assoc-some :children (seq (map second children))))))))

(def input (parse (slurp "resources/day7.txt")))
...
(def schema
  [{:db/ident :program/name
   ...
   {:db/ident :program/weight
    :db/valueType :db.type/long
    :db/cardinality :db.cardinality/one}
   {:db/ident :program/children
    :db/valueType :db.type/ref
    :db/cardinality :db.cardinality/many}])

(defn assoc-when [m test k v]
  (if test (assoc m k v) m))

(defn input-transaction [in]
  (mapv (fn [{:keys [name weight children]}]
          (-> {:db/id name
               :program/name name
               :program/weight weight}
              (comfy/assoc-some :program/children (seq children))))
        in))


(def uri* "datomic:mem://examples")
...
(d/transact conn* (input-transaction (parse example)))


(def uri "datomic:mem://programs-tree")
...
(d/transact conn (input-transaction input))


(defn bottom-program [database]
  (ffirst
   (d/q '[:find ?name
          :where
          [?e :program/name ?name]
...
(deftest one
  (is (= "tknk" (bottom-program (d/db conn*)))))

(comment

  (bottom-program (d/db conn)) ;=> "azqje"
